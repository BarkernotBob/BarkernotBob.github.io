...
| `flows.spec.js` | One smoke test per main flow — mark a task done, log a strip test and a numeric test, back-date a test (#139: keeps its date, sorts by it, never drags the test task backwards, refuses the future), recent activity under a renamed task's current title (#143), log swim time, edit a task, mark a seasonal checklist done, pool and season settings, history, weather, a GitHub failure surfacing, and #134 one-tap-one-record (a failed save retried, a landed save whose reply was lost, two taps inside one round trip on Today and Schedule with the busy button moving nothing, re-logging today asks first) |
| `delete.spec.js` | Issue #138 — deleting a mis-logged entry: an activity row, a test and a swim (its hours go with it, by `swimId` or, for older rows, same hours and moment), each behind a confirm and in ONE commit; a task's `last` date walks back to the newest evidence left (or clears) when the deleted record was what it rested on, and stays when a duplicate is left; the deleted row fades in place and nothing below it moves; a delete racing another device keeps its entry |
...
| `parity.spec.js` | Issue #142 — the daily email and the app give the same advice. Runs the real `reminders.mjs` under Node, copied alone into an empty folder as the workflow's `curl` leaves it (so it must fetch `shared/pool-chem.js` over HTTP), then boots the app on the same config + tests and compares the advice cards line for line: an old config, readings that interact, a blanked target range with no inventory, and a strip test logged as printed values (#137) |
| `strips.spec.js` | Issue #137 — Strips mode logs the numbers printed on the bottle: each pad offers its printed scale plus the half-way values between swatches, a strip test saves into `nums` (combined chlorine, pH at 7.5 in range), a second tap clears, a tap survives a mode switch, tapping moves nothing at 390 and 1300px, two strip tests draw a trend line for free chlorine and pH, old words-only tests still render and advise unmigrated, the AI export carries the numbers, and a pad's scale edits in Settings |
| `location.spec.js` | Issue #165 — Settings location by address or lat/lon: a street address found through Nominatim (Open-Meteo's geocoder as fallback when it is down or finds nothing), Save looking the address up itself when Look up was never tapped, an unfindable address not saved, typed coordinates saved with ± supplying the minus the iPhone keypad lacks, out-of-range or non-numeric values refused, the latlong.net link, and switching modes moving nothing and keeping edits at 390 and 1300px |
| `rates.spec.js` | pool-data#3 — measured rates on Schedule → Water level: an older config's `waterLevel.drainRate` and the fill sentence in the note become rows of `waterLevel.rates` (7–10 min, 1 in / hr, note not repeated), a measurement added from the app saves as another row (decimal keypad, date picker), edit and a single value, a bad value reopening the form with what was typed and saving nothing, delete behind a confirm, and no sideways scroll or movement opening/cancelling Edit at 390 and 1300px |
